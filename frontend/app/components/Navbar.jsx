        ...
        <div className=' w-full p-10 flex flex-row justify-between items-center lg:px-24 md:px-16 px-5'>
            <div className=' flex flex-row justify-center items-center'>
                ...
                <ul className={`lg:flex ${!show ? "hidden" : ''} lg:flex-row flex-col lg:static absolute right-5 top-24 rounded-2xl lg:p-0 p-5 lg:bg-transparent bg-violet-800 gap-8 text-base`}>
                    <Link href={'/home'} className=' hover:text-violet-500 transition-all cursor-pointer hover:scale-105'>
                        HOME
                    </Link>
                    <li className=' hover:text-violet-500 transition-all cursor-pointer hover:scale-105'>
                        SHARE
                    </li>
                    <a target='_blank' href='https://vaxad.vercel.app' className=' hover:text-violet-500 transition-all cursor-pointer hover:scale-105'>
                        ABOUT
                    </a>
                    <a target='_blank' href='https://github.com/vaxad/vShare' className=' hover:text-violet-500 transition-all cursor-pointer hover:scale-105'>
                        REPO
                    </a>
                </ul>
            </div>
            <div className=' flex flex-row gap-4 justify-center items-center'>
                {!authenticated?
                <Link href="/auth" className=' bg-violet-500 transition-all lg:py-4 hover:bg-white hover:text-violet-500 lg:px-6 md:px-4 md:py-3 px-3 py-2 rounded-full lg:text-base md:text-md text-xs'>SIGN UP</Link>
                :
                <button onClick={()=>handleSignOut()} className=' bg-violet-500 transition-all lg:py-4 hover:bg-white hover:text-violet-500 lg:px-6 md:px-4 md:py-3 px-3 py-2 rounded-full lg:text-base md:text-md text-xs'>LOG OUT</button>
                }
